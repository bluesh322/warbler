...
def toggle_like(msg_id):
    """Add like and unlike to user profile"""
    if not g.user:
        flash("Access unauthorized.", "danger")
        return redirect("/")

    is_liked = Likes.query.filter(
        and_(Likes.user_id == g.user.id, Likes.message_id == msg_id)).first()
    if is_liked:
        # unlike
...
def users_likes(user_id):
    ...
    return render_template('users/likes.html', messages=messages, likes=likes, user=user)
...
def homepage():
    ...
    if g.user:
        ...
        messages = (Message
                    .query
                    .filter(or_(Message.user_id.in_(is_following), (Message.user_id == g.user.id)))
                    .order_by(Message.timestamp.desc())
                    .limit(100)
                    .all())
...
##############################################################################
# Turn off all caching in Flaske
#   handled elsewhere)
#
